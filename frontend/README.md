...
| `/` | 통합 대시보드 | 정책 **카드 그리드** — 활성·스케줄 배지, 최근 실행(시각·상태·그룹 수·경고), **미분석 신규 그룹 수(강조)**, 24h 오류 건수, 바로가기(실행[admin]·대시보드·그룹 보기·정책 설정). 검색 + 정렬(기본 **미분석 많은 순**) |
...
| `/policies` | 분석 정책 관리 | 목록(활성·비활성 모두, 배지 표시) + 생성/수정 폼(**활성 토글**, **스케줄 토글·주기·신규 그룹 자동 분석**), 연결 테스트, **저장 전 미리보기**, 행 선택 시 **상세 + 실행 이력**(수동/자동 배지). `?policy=<id>` 로 특정 정책을 열 수 있다 (통합 대시보드 카드에서 진입) |
